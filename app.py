...
# --- 🎯 ตั้งค่าพิกัดข้อความ (PDF CONFIG) อิงจากเอกสารจริง ---
PDF_CONFIG = {
    "faculty":    (130, 770),  # หน่วยงาน
    "doc_no":     (90, 750),   # ที่ มพย
    "date_day":   (370, 750),  # วันที่
    "date_month": (420, 750),  # เดือน
    "date_year":  (490, 750),  # พ.ศ.
    "subject":    (90, 725),   # เรื่อง
    "to_who":     (90, 700),   # เรียน
    "attach_1":   (130, 675),  # สิ่งที่ส่งมาด้วย
    "check_req":  (75, 642),   # [X] ขอเบิกเงิน
    "amount":     (190, 615),  # จำนวนเงินตัวเลข
    "amount_txt": (330, 615),  # จำนวนเงินตัวอักษร
    "pay_to":     (130, 580),  # สั่งจ่ายให้
    "req_d":      (340, 580),  # รับเงินวันที่
    "req_m":      (390, 580),  # รับเงินเดือน
    "req_y":      (470, 580),  # รับเงิน พ.ศ.
    "check_bank": (75, 528),   # [X] เข้าบัญชีธนาคาร
    "bank_detail":(250, 528),  # ชื่อและเลขบัญชี
    "project":    (200, 502),  # ใช้ในกิจกรรมดังนี้
    "faculty_budget": (240, 472), # งบประมาณของหน่วยงาน
    "check_budget": (75, 442), # [X] ประเภทงบ
    "budget_cat": (160, 442),  # หมวดงบประมาณ
    "leader":     (370, 335),  # ชื่อผู้เบิกเงิน
    "position":   (370, 310),  # ตำแหน่ง
}
...
FACULTY_MASTER = [
    ...
    "สำนักวิจัย", "สำนักบริการวิชาการ"  # เพิ่ม 2 รายการนี้ให้แล้วครับ
]
...
def create_filled_pdf(data):
    ...
    def draw(key, text):
        can.drawString(PDF_CONFIG[key][0], PDF_CONFIG[key][1], str(text))
    ...
    # --- ติ๊กช่องขอเบิกเงิน และ ยอดเงิน ---
